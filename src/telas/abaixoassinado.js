...
import {  Image, View, Text, StyleSheet,  Dimensions, TouchableWithoutFeedback, ActivityIndicator, ScrollView, Alert, Linking} from 'react-native'
    ...
    assinar= async () =>{
        ...
        let usuario = await AsyncStorage.getItem('USUARIO')
            ...
            if( (!usuario.hasOwnProperty('email')) || 
                ...
                if(abaixo.hasOwnProperty('assinantes')) delete abaixo.assinantes;
                firebase.database().ref('usuarios/' + usuario.uid + '/participacoes/' + this.state.abaixo.id).update({ ...abaixo })

                }
       
    }
     onShareFace = async(event) => {
        
 /*       const shareOptions = {
            title: 'Compartilhamento de abaixo-assinado',
            message: `${this.state.abaixo.nome}`+ "\n" +`Compartilhe essa ideia. ✍`+ "\n" +`Faça a diferença você também!`+ "\n",
            url: `https://${this.state.link}${this.state.abaixo.id}`,
            social: Share.Social.FACEBOOK,
   //         whatsAppNumber: "21993984342",  // country code + phone number(currently only works on Android)
...
*/
        ...
        const shareLinkContent = {
            contentType: 'link',
            contentUrl: `https://${this.state.link}${this.state.abaixo.id}`,
   //         contentUrl: this.state.link  + this.state.abaixo.id,
            quote: `${this.state.abaixo.nome}`+ "\n" +`Compartilhe essa ideia. ✍`+ "\n" +`Faça a diferença você também!`+ "\n",
          };
          var tmp = this;
  ...
  //          filename: 'test' , // only for base64 file in Android 
          ...
          await Share.shareSingle(shareOptions).then(
            function(canShow) {
                Alert.alert(
                    `${canShow}`,
                    'Obrigado por Compartilhar',
              ...
              if (canShow) {
                return Share.show(shareOptions);
              }
            }
          ).then(
            function(result) {
    ...
    render() {
       ...
       if(this.state.shareface)
            return(
                <WebView
                    onNavigationStateChange={(event)=>{this.onShareFace(event)}}
           ...
           <View style={styles.container}>
               <View style={{
                   ...
                   <View style={{
                       ...
                       <TouchableWithoutFeedback onPress={() => this.onShareFace()} >
                           
                           <Image style={styles.iconefacebook} resizeMode='stretch' source={require('../../assets/facebook.png')} />
                       </TouchableWithoutFeedback>
                       <TouchableWithoutFeedback onPress={() => this.onShareWhat()} >
                          
                          <Image style={styles.iconewhatsapp} resizeMode='cover' source={require('../../assets/Whatsapp.png')} />
                       </TouchableWithoutFeedback>
                       {
