...
import { View,Text ,StyleSheet,Button,ActivityIndicator, Dimensions,ScrollView , Alert} from 'react-native'
...
class Pagamento extends Component{
    ...
    static navigationOptions = ({ navigation }) => {
      ...
      return {
          header: <Header titulo={'Pagamento'}  navigation={navigation} back={true} menu={true} setting={false} />
      }
    ...
    onMessage= async ( event )=>{
      ...
      try{
        ...
        if(respostadaWebView.hasOwnProperty('error') && (respostadaWebView.error === true)){
          let msg = ''
          for (const key in respostadaWebView.errors) {
            if (respostadaWebView.errors.hasOwnProperty(key)) {
              const erropagseguro = respostadaWebView.errors[key];
              msg+=erropagseguro + ' '
    ...
    componentWillReceiveProps = (nxtprops) =>{
      if (nxtprops.mensagem === 'Pagamento Feito Com Sucesso'){
        this.props.navigation.navigate('Impulsionar')
      }
    }
    criarCardToken = async () =>{
    ...
    render(){
        return(
            <ScrollView style={{flex:1, marginTop: height * 0.029,}}>            
             <View style={{
            ...
            <View style={{opacity:0,flex:1,position:'absolute',top:-height}}>
            <WebView 
  ...
  window.postMessage = function(data) {
    window.ReactNativeWebView.postMessage(data);
            ...
            <View style={{
              ...
              <Text>Valor:R$ {this.props.itemAmount1}</Text>
            </View>
<CreditCardInput
              autoFocus
              name={''}
