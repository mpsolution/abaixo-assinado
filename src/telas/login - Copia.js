                ...
                if (user != null) {
                    ...
                    firebase.database().ref('/usuarios/'+user.providerData[0].uid).once('value',async (snapshot)=>{
                        ...
                        await  AsyncStorage.setItem('USUARIO', JSON.stringify({...usuario}))
                            ...
                            firebase.database().ref('/usuarios/' + user.providerData[0].uid).update({ uid:user.providerData[0].uid,nome:user.displayName ,displayName:user.displayName,email:user.email,photoURL:user.photoURL })
                            await AsyncStorage.setItem('USUARIO', JSON.stringify({ uid:user.providerData[0].uid,nome:user.displayName ,displayName:user.displayName,email:user.email,photoURL:user.photoURL}))
                            this.props.navigation.navigate("Inicio", { usuario:{  uid:user.providerData[0].uid,nome:user.displayName ,displayName:user.displayName,email:user.email,photoURL:user.photoURL} })
                            try{
                                AsyncStorage.setItem('credential', credential)
                            }catch(error){
                                console.log(error)
